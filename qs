...
(define (analyse-sql! ssql queue seen)
  (define (->temporary-table table)
    (match table
      (('! t)
       (let ((table (string->symbol t)))
	 (cond ((hashtable-ref seen table #f))
	       (else
		(let ((temp (make-temporary-table)))
		  (hashtable-set! seen table temp)
		  (list-queue-add-front! queue (cons table temp))
		  temp)))))
      ((? stdin?)
       (cond ((hashtable-ref seen +stdin+ #f))
	     (else
	      (let ((temp (make-temporary-table)))
		(hashtable-set! seen table temp)
		(list-queue-add-front! queue (cons table temp))
  ...
  (match ssql
    ...
    ((a . d) (cons (analyse-sql! a queue seen) (analyse-sql! d queue seen)))
    (v v)))
...
(define (populate-tables dbi-conn skip-header queue seen)
  ...
  (define (populate-table info)
    (let ((file (car info))
	  (table (cdr info)))
      (unless (eq? (hashtable-ref seen file) 'done)
	(hashtable-set! seen file 'done)
	(if (eq? file +stdin+)
  ...
  (with-args (cdr args)
    ...
    (let* ((sql (car rest))
	 ...
	 (let* ((table-queue (list-queue))
		(invoking-sql (analyse-sql! s table-queue seen)))
